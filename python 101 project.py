...
def NumberSearch(con,number):
    if len(uinput) == 10 and number.isdigit(): # isdigit() = one of string methods
        for number , name in con.items(): # .items() = one of dictionary methods 
            if uinput == number:
                return name
        else:
